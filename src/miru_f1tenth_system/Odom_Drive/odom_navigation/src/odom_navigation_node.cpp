...
class OdomNavigationNode : public rclcpp::Node
{
  public:
    OdomNavigationNode()
        : Node("odom_navigation_node"), target_set_(false), goal_reached_(false), is_active_(false), origin_set_(false),
          prev_cte_(0.0), integral_cte_(0.0), prev_speed_error_(0.0), integral_speed_error_(0.0)
    {
  ...
  private:
        ...
        if (is_active_ != was_active)
        {
            if (is_active_)
            {
...

                {
                    ...
                    RCLCPP_INFO(this->get_logger(), "New target set with offset (%.2f, %.2f)", target_x_, target_y_);
                }
            }
            else
            {
    ...
    void odomCallback(const odom_msgs::msg::MyOdom::SharedPtr msg)
    {
        ...
        if (is_active_ && !origin_set_)
        {
            ...
            origin_yaw_ = msg->yaw;
            origin_set_ = true;
            RCLCPP_INFO(this->get_logger(), "Set origin position for Mission C at (%.2f, %.2f, yaw: %.2f)", origin_x_,
                        origin_y_, origin_yaw_);
        }
        ...
        if (!target_set_)
        {
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "Target not set. Waiting for user input...");
            return;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Current pos: (%.2f, %.2f), Target: (%.2f, %.2f), Distance: %.2f", x, y,
                    target_x_, target_y_, distance);
        ...
        double transformed_x =
            std::cos(origin_yaw_rad - current_yaw_rad) * rel_x - std::sin(origin_yaw_rad - current_yaw_rad) * rel_y;
        double transformed_y =
            std::sin(origin_yaw_rad - current_yaw_rad) * rel_x + std::cos(origin_yaw_rad - current_yaw_rad) * rel_y;
        ...
        double speed_cmd =
            kp_speed_pid_ * speed_error + ki_speed_pid_ * integral_speed_error_ + kd_speed_pid_ * diff_speed_error;
